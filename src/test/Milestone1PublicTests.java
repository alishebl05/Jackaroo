...
public class Milestone1PublicTests {
	...
	//	drawCards() Method returns the first four view.cards
	@Test(timeout=1000) 
	public void testDrawCardsInDeckReturnsFourCards() {
		...
		try {
			...
			assertEquals("Returned list should contain exactly 4 view.cards",4, returnedList.size());
	...
	public void testDrawCardsInDeckRemovedCardsCards() {
		...
		try {
			...
			for (int i = 0; i < returnedList.size(); i++) {
				assertFalse("Returned view.cards should be removed from the view.cards pool", cardsPool_Object.contains(returnedList.get(i)));
			}
			assertEquals("Cards pool list should only be missing 4 view.cards, size is incorrect", 6,cardsPool_Object.size() );
	...
	public void testGameConstructorInitializationHandPlayers() throws Exception{
		...
		for (int i = 0; i < myPlayers.size(); i++) {
			...
			assertEquals("Player's hand should countain exactly 4 view.cards", 4, handPlayers.size());
		...
		assertTrue("Each player in the players list should be assigned a unique hand. make sure that loadCardPool create"
				+ " unique view.cards and not copy the same view.cards according to the frequency"
				,handSet.size()==4);
	...
	public void testCardInstanceVariableNameisFinal() throws NoSuchFieldException, SecurityException, ClassNotFoundException {
		Field nameField = Class.forName(cardPath).getDeclaredField("name");
		assertTrue("The name instance variable in class Card should be final", Modifier.isFinal(nameField.getModifiers()));
	}
	...
	public void testCardInstanceVariableDescriptionisFinal() throws NoSuchFieldException, SecurityException, ClassNotFoundException {
		Field descriptionField = Class.forName(cardPath).getDeclaredField("description");
		assertTrue("The description instance variable in class Card should be final", Modifier.isFinal(descriptionField.getModifiers()));
	}
	...
	public void testCardInstanceVariableSuitiFinal() throws NoSuchFieldException, SecurityException, ClassNotFoundException {
		Field suitField = Class.forName(standardPath).getDeclaredField("suit");
		assertTrue("The suit instance variable in class Card should be final", Modifier.isFinal(suitField.getModifiers()));
	}
