...
public class Board implements BoardManager {
    ...
    private void move(Marble marble, ArrayList<Cell> fullPath, boolean destroy) throws IllegalDestroyException {
        ...
        if(targetCell.isTrap()) {
            targetCell.trapActivated = marble.getColour();
    ...
    public int getPositionInPathPublic(ArrayList<Cell> path, Marble marble) {
        for(int i = 0; i < path.size(); i++) {
            if(path.get(i).getMarble() == marble)
                return i;
        }
    ...
    public ArrayList<Cell> getSafeZonePublic(Colour colour) {
        for (int i = 0; i < 4; i++)
            if (this.safeZones.get(i).getColour() == colour)
                return this.safeZones.get(i).getCells();
